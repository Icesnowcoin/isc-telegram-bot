...
async def fetch_price():
    """获取 ISC 实时价格 - DexScreener + PancakeSwap Subgraph 双源"""
    global price_cache
            ...
            async with session.get(url, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    if pairs:
                        ...
                        price_cache = {"data": result, "timestamp": now}
                        return result
    except Exception as e:
        logger.warning(f"DexScreener 价格获取失败: {e}")

    # 源2: PancakeSwap V3 Subgraph (BSC)
    try:
        async with aiohttp.ClientSession() as session:
            ...
            query = {
                ...
                """,
                "variables": {"token": ISC_CONTRACT.lower()}
            }
            async with session.post(
                subgraph_url, json=query, timeout=10,
                headers={"Content-Type": "application/json"}
            ) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    token_data = data.get("data", {}).get("token")
                    if token_data and token_data.get("derivedUSD"):
                        result = {
                            "price": f"{float(token_data['derivedUSD']):.8f}",
                            "change24h": "N/A",
                            "volume24h": f"{float(token_data.get('volumeUSD', 0)):,.2f}",
                            "liquidity": f"{float(token_data.get('totalValueLockedUSD', 0)):,.2f}",
                            "fdv": "N/A",
        ...
        logger.warning(f"PancakeSwap Subgraph 价格获取失败: {e}")
...
ISC 目前仅在 PancakeSwap 去中心化交易所交易，第三方价格聚合平台可能尚未收录。

请直接通过以下平台查看实时行情：
🔗 <a href="{PANCAKESWAP_URL}">PancakeSwap 交易</a>
🔗 <a href="https://dexscreener.com/bsc/{ISC_CONTRACT}">DEXScreener 图表</a>
🔗 <a href="{BSCSCAN_URL}">BSCScan 代币页</a>

<b>合约地址：</b><code>{ISC_CONTRACT}</code>
        """
...
def main():
    ...
    logger.info("🤖 ISC Bot v2.1 已启动！DexScreener + PancakeSwap Subgraph 双源价格查询就绪")
    print("🤖 ISC Bot v2.1 已启动！")
